	...
	for _, NsRwSet := range txRWSet.NsRwSets{
		if NsRwSet.NameSpace == "bscc"{
			...
			for _, write := range NsRwSet.KvRwSet.Writes{
				...
				temBalance, ok := allCertBalance[write.Key]
				if ok{
					err := checkAndUpdateCertBalance(allCertBalance, write.Key, spend,
						temBalance, write)
					if err != nil{
						return err
					}
				}else{
					allCertBalance[write.Key] = certBalance
					temBalance, _ := allCertBalance[write.Key]
					err := checkAndUpdateCertBalance(allCertBalance, write.Key, spend,
						temBalance, write)
					if err != nil{
						return err
...
func checkAndUpdateCertBalance(allCertBalance map[string]string, key string, spend string,
